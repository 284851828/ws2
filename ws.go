...
type Manager struct {
	...
	ReceiveMsgHandle        func(id, group string, msg []byte)
}
...
type Client struct {
	...
	Message      chan []byte
	DoMsg        func(id, group string, msg []byte)
	DoDisconnect func(c *Client, id, group string)
}
...
func (c *Client) Read() {
	...
	for {
		...
		// log.Printf(" ReadMessage  messageType %d , message, err %v ", messageType, err)
		if err != nil || messageType == websocket.CloseMessage {
			break
		}
		// log.Printf("client [%s] receive message: %s", c.Id, string(message))
		if c.DoMsg != nil {
			c.DoMsg(c.Id, c.Group, message)
		} else {
			c.Message <- message
		}
	}
	...
	for {
		...
		case message, ok := <-c.Message:
			...
			if err != nil {
				...
				if c.DoDisconnect != nil {
					c.DoDisconnect(c, c.Id, c.Group)
				}
			}
		}
	}
...
func (manager *Manager) Remove(client *Client, id, group string) {
	manager.Lock.Lock()
	if _, ok := manager.Group[client.Group]; ok {
		if _, ok := manager.Group[client.Group][client.Id]; ok {
			close(client.Message)
...
func (manager *Manager) WsClient(ctx *gin.Context) {
	...
	client := &Client{
		...
		DoMsg:        manager.ReceiveMsgHandle,
		DoDisconnect: manager.Remove,
	}
